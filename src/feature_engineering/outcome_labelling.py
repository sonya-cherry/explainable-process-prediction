...
# ---------------------------------------------------------------------------
# Sprint 1 legacy helpers
#
# These implement the initial duration-based outcome definition. They are no
# longer used by the production pipeline (which uses the rule-based ``outcome``
# above), but the exploration and baseline notebooks (1 and 2) still rely on
# them, so they are kept here as the single source of truth.
# ---------------------------------------------------------------------------
def compute_duration_outcomes(
    ...
    Positive outcome: case duration is less than or equal to the median case
    duration. Negative outcome: case duration is greater than the median.
...
def tempFormat(df: pd.DataFrame, dur: pd.DataFrame) -> pd.DataFrame:
    """Merge the case-level outcome from ``compute_duration_outcomes`` back onto
    every event row of the original DataFrame."""
    return df.merge(
        dur[["case:concept:name", "outcome"]],
